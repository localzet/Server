    ...
    public static function input($buffer, TcpConnection $connection)
    {
        ...
        if ($connection->websocketCurrentFrameLength) {
            ...
            if ($total_package_size > $connection->maxPackageSize) {
                Server::safeEcho("error package. package_length=$total_package_size\n");
                $connection->close();
                return 0;
            }
        ...
        if ($connection->handshakeStep === 1) {
            ...
            if (\strlen($connection->tmpWebsocketData) > $connection->maxSendBufferSize) {
                if ($connection->onError) {
                    try {
                        \call_user_func($connection->onError, $connection, \V3_SEND_FAIL, 'send buffer full and drop package');
                    } catch (\Exception $e) {
                        Server::stopAll(250, $e);
                    } catch (\Error $e) {
                        Server::stopAll(250, $e);
        ...
        if ($pos) {
            ...
            if (\preg_match("/Sec-WebSocket-Accept: *(.*?)\r\n/i", $buffer, $match)) {
                if ($match[1] !== \base64_encode(\sha1($connection->websocketSecKey . "258EAFA5-E914-47DA-95CA-C5AB0DC85B11", true))) {
                    Server::safeEcho("Sec-WebSocket-Accept not match. Header:\n" . \substr($buffer, 0, $pos) . "\n");
                    $connection->close();
                    return 0;
                }
            } else {
                Server::safeEcho("Sec-WebSocket-Accept not found. Header:\n" . \substr($buffer, 0, $pos) . "\n");
                $connection->close();
                return 0;
            }
